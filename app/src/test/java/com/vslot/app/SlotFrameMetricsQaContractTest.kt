...
class SlotFrameMetricsQaContractTest {
    ...
    fun `instrumentation test measures a deterministic spin and reports stable percentiles`() {
        ...
        assertTrue(source.contains("PHYSICAL_SAMSUNG_MAX_FRAME_NANOS = 150L * NANOS_PER_MILLISECOND"))
        assertTrue(source.contains("PHYSICAL_SAMSUNG_MAX_JANK_RATE_PERCENT = 10.0"))
        assertTrue(source.contains("PHYSICAL_SAMSUNG_MAX_MISSED_DEADLINE_RATE_PERCENT = 25.0"))
        ...
        assertTrue(source.contains("removeFrameMetricsListenerIfAttached(listener)"))
    ...
    fun `host script targets one serial runs only frame test and redacts evidence`() {
        ...
        assertTrue(script.contains("missed_deadline_limit_pct=25.0"))
        assertTrue(script.contains("max_limit_ms=150.0"))
        ...
        assertTrue(script.contains("settings get global stay_on_while_plugged_in"))
        assertTrue(script.contains("settings put global stay_on_while_plugged_in 7"))
        assertTrue(script.contains("restore_setting_and_verify global stay_on_while_plugged_in"))
        assertTrue(script.contains("for attempt in 1 2 3"))
        assertTrue(script.contains("input keyevent KEYCODE_WAKEUP"))
        assertTrue(script.contains("wm dismiss-keyguard"))
        assertTrue(script.contains("Unlock \$model before running slot frame-metrics QA."))
