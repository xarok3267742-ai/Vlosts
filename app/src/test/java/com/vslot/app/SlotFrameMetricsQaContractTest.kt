...
class SlotFrameMetricsQaContractTest {
    ...
    fun `instrumentation test measures a deterministic spin and reports stable percentiles`() {
        ...
        assertTrue(source.contains("EMULATOR_P95_LIMIT_NANOS = 125L * NANOS_PER_MILLISECOND"))
        assertTrue(source.contains("EMULATOR_MAX_FRAME_NANOS = 250L * NANOS_PER_MILLISECOND"))
        assertTrue(source.contains("EMULATOR_MAX_JANK_RATE_PERCENT = 85.0"))
        assertTrue(source.contains("EMULATOR_MAX_MISSED_DEADLINE_RATE_PERCENT = 90.0"))
        ...
        assertTrue(source.contains("PHYSICAL_SAMSUNG_MAX_FRAME_NANOS = 250L * NANOS_PER_MILLISECOND"))
        ...
        assertTrue(source.contains("maxJankRatePercent = PHYSICAL_SAMSUNG_MAX_JANK_RATE_PERCENT"))
        ...
        assertTrue(source.contains("p95Nanos <= limits.p95Nanos"))
        assertTrue(source.contains("p99Nanos <= limits.p99Nanos"))
        assertTrue(source.contains("maxNanos <= limits.maxNanos"))
        assertTrue(source.contains("jankRatePercent <= limits.maxJankRatePercent"))
        assertTrue(source.contains("missedDeadlineRatePercent <= limits.maxMissedDeadlineRatePercent"))
    }
    ...
    fun `host script targets one serial runs only frame test and redacts evidence`() {
        ...
        assertTrue(script.contains("local p95_limit_ms=125.0"))
        assertTrue(script.contains("local max_limit_ms=250.0"))
        assertTrue(script.contains("local jank_rate_limit_pct=85.0"))
        assertTrue(script.contains("local missed_deadline_limit_pct=90.0"))
