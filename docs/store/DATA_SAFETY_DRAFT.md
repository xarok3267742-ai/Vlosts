...
Статус: **не отправлять в Play Console без закрытия пунктов ниже**. Черновик составлен по исходному коду и прямым runtime-зависимостям на 2026-08-04; требования Google Play Data safety повторно сверены 2026-08-04. Он не знает production API keys, dashboard settings, подтверждённые сроки хранения error-log у privacy host, договорные роли провайдеров и реальные сетевые ответы release AAB.
...
- Firebase Cloud Messaging `25.1.2` и AppMetrica Push `4.3.0` используются для уведомлений. До системного `POST_NOTIFICATIONS` prompt приложение показывает русский pre-prompt, который называет Firebase (Google) и AppMetrica (Яндекс), идентификаторы приложения/устройства и телеметрию доставки уведомлений. Firebase auto-init выключен в manifest и включается только после сохранённого действия пользователя, разрешения `POST_NOTIFICATIONS` и включённых системных уведомлений. При отзыве после запроса код выключает auto-init и удаляет FCM token вместе с Firebase Installation ID; это не удаляет ранее обработанные данные провайдеров.
...
- Privacy policy открывается в ограниченном WebView только по HTTPS; navigation и subresources разрешены лишь на том же origin. Cookies, JavaScript, DOM storage, file/content access и mixed content выключены. Production route подготовлен как статический Nginx endpoint с `access_log off`, без внешних ресурсов и активного контента; до заполнения финальной политики он намеренно возвращает `404`. Сервер, TLS-инфраструктура и хостинг-провайдер всё равно могут обрабатывать сетевые метаданные, включая IP и User-Agent; сроки error-log и договорную роль нужно подтвердить отдельно.
...
- [ ] **Privacy WebView host:** route и CSP проверены, cookies/redirect на другой origin/внешние ресурсы отсутствуют, а Nginx `access_log` для endpoint выключен. До sign-off подтвердить error-log retention, инфраструктурные логи, отсутствие CDN/proxy-сбора, роль и subprocessors хостинг-провайдера; затем зафиксировать те же факты в финальной политике.
