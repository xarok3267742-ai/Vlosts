...
- [ ] Указать настоящий публичный `V_SLOT_PRIVACY_POLICY_URL`. Страница должна быть HTTPS, без геоблокировки, не PDF, доступна без входа и явно называть V Slot и юридическое лицо/разработчика из store listing. Google требует privacy policy в Play Console и внутри приложения; она должна описывать сбор, использование, передачу, безопасность, сроки хранения и удаление данных: [Google Play User Data policy](https://support.google.com/googleplay/android-developer/answer/17105854?hl=en#privacy_policy).
  - [x] На предоставленном сервере подготовлен HTTPS-маршрут `https://139-100-227-46.nip.io/v-slot/privacy`, отдельный непривилегированный пользователь `vslotdeploy` и принадлежащий ему каталог `/var/www/v-slot`. До публикации проверенного документа маршрут намеренно отвечает `404`.
...
- [x] Создан отдельный RSA-4096 Google Play upload key вне Git-репозитория. Публичные параметры и точный certificate SHA-256 закреплены в [`upload-key-certificate.json`](upload-key-certificate.json); приватный JKS имеет owner-only permissions, а разные store/key passwords сохранены в macOS Keychain без доверенных приложений. Повторная генерация и перезапись существующего ключа запрещены [`create_play_upload_key.sh`](../../tools/create_play_upload_key.sh).
- [ ] До первой загрузки сохранить зашифрованную резервную копию JKS и оба секрета отдельно от этого Mac, документировать владельца и процедуру восстановления, затем зарегистрировать закреплённый upload certificate в Play Console. Для локальной сборки использовать [`with_play_upload_key.sh`](../../tools/with_play_upload_key.sh): wrapper сверяет владельца, permissions, ACL и certificate SHA-256, разрешает только repository `gradlew` и принудительно включает `--no-daemon`. При системном запросе Keychain выбирать одноразовый `Allow`, а не `Always Allow`; CI должен передавать те же пять `V_SLOT_RELEASE_*` значений через собственный secret storage.
...
- [ ] **Content rating:** заполнить IARC честно: simulated gambling/virtual slots присутствуют; real-money gambling, purchases и cash prizes отсутствуют. Не назначать рейтинг вручную и не предполагать, что in-app `18+` автоматически равен рейтингу IARC: [Content rating requirements](https://support.google.com/googleplay/android-developer/answer/9859655?hl=en).
  ...
  После получения сертификата заполнить копию [IARC_EVIDENCE_TEMPLATE.json](IARC_EVIDENCE_TEMPLATE.json), вычислить SHA-256 и сохранить JSON как защищённый GitHub secret `V_SLOT_IARC_EVIDENCE_JSON_BASE64`, а хэш как production variable `V_SLOT_IARC_EVIDENCE_SHA256`. Не коммитить заполненный файл: release workflow валидирует точные ответы, package, versionCode и сертификат, а наружу публикует только обезличенный `iarc-validation.txt`.
...
```bash
tools/with_play_upload_key.sh ./gradlew clean :app:verifyStoreRelease :app:bundleRelease
```
