...
SOURCE = ROOT / "qa/source/vslot_theme_reel_spin_blur_imagegen.png"
OUT_DIR = ROOT / "app/src/main/res/drawable-nodpi"
PREVIEW = ROOT / "qa/screenshots/theme_reel_spin_blur_contact_sheet.png"
MOCKUP = ROOT / "qa/screenshots/theme_reel_spin_blur_on_symbols_contact_sheet.png"

TARGET_SIZE = (960, 960)
GRID_COLUMNS = 5

CELLS = [
    ("reel_spin_blur_violet", "Violet", "vf_symbol_diamond.webp", "vf_symbol_v_wild.webp", "vf_symbol_star.webp"),
    ("reel_spin_blur_roman", "Roman", "rr_symbol_laurel.webp", "rr_symbol_v_wild.webp", "rr_symbol_coin.webp"),
    ("reel_spin_blur_neon", "Neon", "nn_symbol_neon_seven.webp", "nn_symbol_v_wild.webp", "nn_symbol_holo_chip.webp"),
    ("reel_spin_blur_pharaoh", "Pharaoh", "pg_symbol_scarab.webp", "pg_symbol_v_wild.webp", "pg_symbol_ankh.webp"),
    ("reel_spin_blur_ocean", "Ocean", "op_symbol_shell.webp", "op_symbol_v_wild.webp", "op_symbol_pearl.webp"),
]
...
def alpha_from_luminance(image: Image.Image) -> Image.Image:
    ...
    alpha = luminance.point(
        lambda value: 0
        if value < 6
        else min(210, int(((value - 6) / 132) ** 1.16 * 255))
    )
...
def fade_outer_frame(alpha: Image.Image) -> Image.Image:
    ...
    return alpha.filter(ImageFilter.GaussianBlur(radius=0.15))
...
def build_mockup(panels: list[tuple[str, Image.Image]]) -> None:
    ...
    for index, ((label, blur), cell) in enumerate(zip(panels, CELLS)):
        x = gap + index * (tile + gap)
        y = gap
        draw.text((x, y), label, fill=(246, 240, 255))
        base = symbol_grid(cell[2:], (tile, tile))
        blur_preview = blur.copy()
        blur_preview.thumbnail((tile, tile), Image.Resampling.LANCZOS)
        base.alpha_composite(blur_preview, ((tile - blur_preview.width) // 2, (tile - blur_preview.height) // 2))
